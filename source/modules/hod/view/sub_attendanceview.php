...
<form method="post" class="form-group" enctype="multipart/form-data">
	...
	<select name="class"  id="class" onChange="showsub(this.value)"   class="form-control">
		...
		<?php
		$res=mysqli_query($conn,"select * from department where hod='$st'");

		if($rs=mysqli_fetch_array($res))
		{
			$res1=mysqli_query($conn,"select *from class_details where deptname='$rs[deptname]' and active='YES'");
...
//	$student=explode("-",$class[4]);
			...
			<table class="table table-hover table-bordered" >
				...
				<tr>
					...
					<th>Name</th>
					<?php
					$res=mysqli_query($conn,"select * from subject_class where classid='$class[0]' order by subjectid asc");
...
//	{ 
					?><?php
//		$res2=mysqli_query($conn,"SELECT a.adm_no,b.name,c.rollno FROM stud_sem_registration a,stud_details b,current_class c where a.classid='$class[0]' and a.new_sem='$class[2]' and a.adm_no=b.admissionno and a.classid=c.classid and a.adm_no=c.studid order by c.rollno asc");
					$res2=mysqli_query($conn,"SELECT c.studid,b.name,c.rollno FROM stud_details b,current_class c where c.classid='$class[0]' and c.studid=b.admissionno order by c.rollno asc");
					...
					while($rs2=mysqli_fetch_array($res2))
					{
						...
						<tr>
							...
							$total=0;
							$present=0;
							$res3=mysqli_query($conn,"select * from subject_class where classid='$class[0]' order by subjectid asc");
							while($rs3=mysqli_fetch_array($res3))
							{

								$res4=mysqli_query($conn,"select distinct date,subjectid,hour from attendance where studid='$rs2[studid]' and date BETWEEN '$date1' AND '$date2' and subjectid='$rs3[subjectid]' and classid='$class[0]' and ( status = 'P' OR status = 'A' )");
								$res5=mysqli_query($conn,"select distinct date,subjectid,hour from attendance where studid='$rs2[studid]' and date BETWEEN '$date1' AND '$date2' and subjectid='$rs3[subjectid]' and classid='$class[0]' and status='P'");
								?>
								<td><?php 
								if(mysqli_num_rows($res4)==0){
									if( $rs3['type'] == 'ELECTIVE') {
										$tro ="SELECT * FROM elective_student WHERE sub_code = '$rs3[subjectid]' AND stud_id ='$rs2[studid]' ";
										$res49=mysqli_query($conn, $tro);
...
//	$class=explode(",",$_REQUEST['class']);	
 //$class=explode(",",$_POST['class']);
			...
			<?php
			$c=mysqli_query($conn,"select * from subject_class where classid='$class[0]' order by subjectid asc");
