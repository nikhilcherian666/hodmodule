...
<div id="page-wrapper">
  ...
  <div class="row">
    <div class="col-lg-12">
      ...
      <form id="suballoc" action = "../controller/sub_alloc_update.php" method = "POST" enctype = "">
<input type="hidden" name="oldfid" value="<?php echo $staffid; ?>">
      ...
      <td>
       <div id ="data2">
        <?php
        $sql="select * from faculty_details where fid='$staffid'";   
        $r=$conn->query($sql);
        $re=$r->fetch_assoc();
        $n=$re["name"]; 
        ?>
        ...
        <datalist id="name">
         ...
         $l=$conn->query("select deptname from department where hod='$hodid'");
         $r=$l->fetch_assoc();
         $dept=$r["deptname"];
         $sql="select * from faculty_details where name like '$name%' ";  
          $r=$conn->query($sql);
         while($result=$r->fetch_array()){ 
