...
/**
 * @Author: indran
 * @Date:   2018-07-19 13:14:33
 * @Last Modified by:   indran
 * @Last Modified time: 2018-08-11 06:21:05
 */
?>
...
//reading registration details
            $resul=$conn->query("select reg_id,adm_no,classid,apl_status,previous_sem,new_sem,apv_status,remarks from stud_sem_registration where classid='$classid' and (apv_status='Rejected by office' or apv_status='Approved by staff advisor')");

            while($dat=$resul->fetch_array())
            {
                $adm_no=$dat["adm_no"];
                ...
                $result1=$con->query("select * from stud_details where admissionno='$adm_no'");   

                while($dat=$result1->fetch_array())
                {
                    $name=$dat["name"];
                }
                if($apl_status=='Applied' || $apv_status!='Rejected by HOD' )
                {
                    ?>
                    <tr align="center">
                        ...
                        <?php
                        if($apv_status!="Approved by staff advisor")
                         { ?>
                           <td><?php echo $apv_status;?>
                           <br><div class="btn-group">
                            <a  onclick="return confirm('Do you really want to verify this request?');"  href="semregedit.php?id=<?php echo $reg_id;?>&id2=1&id3=<?php echo $classid ?>"><font color="green">Verify</font></a> | 
...
//reading registration details
        ...
        //fetching data from stud_details
                                $result1=$conn->query("select * from stud_details where admissionno='$adm_no'");   
                                while($dat=$result1->fetch_array())  {
                                    $name=$dat["name"];
                                }
